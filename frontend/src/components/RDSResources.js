...
import { Table, Loader, ScrollArea, Input } from '@mantine/core'

export default function RDSResources(props) {
    ...
    React.useEffect(() => {
        async function fetchEC2Resources() {
            const response = await axios.get(
                'http://localhost:4000/api/rds/instances'
            )
    ...
    const rows = resources.filter((ele)=>ele.instance_type.includes(filterSring)).map((element) => (
        ...
        >
            ...
            <Table.Td>{element.memory}</Table.Td>
        </Table.Tr>
    ))
